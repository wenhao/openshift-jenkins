...
doDownload() {
    ...
    curl --connect-timeout "${CURL_CONNECTION_TIMEOUT:-100}" --retry "${CURL_RETRY:-5}" --retry-delay "${CURL_RETRY_DELAY:-0}" --retry-max-time "${CURL_RETRY_MAX_TIME:-60}" -s -f -L "$url" -o "$jpi"
    return $?
}
