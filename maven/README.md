...
passing a Maven command to `docker run`:
    ...
    docker run -it --rm --name my-maven-project -v "$(pwd)":/usr/src/mymaven -w /usr/src/mymaven maven:3.5.0 mvn clean install
...
or run all the tests with
    ...
    for tag in 3.5.0; do TAG=$tag bats tests; done
